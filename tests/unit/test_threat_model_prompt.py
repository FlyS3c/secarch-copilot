"""Tests for the threat-model prompt builder."""
...
def test_required_threat_model_fields_are_named() -> None:
    prompt = build_threat_model_prompt(
        system_description="Synthetic architecture description.",
        evidence_blocks=["SOURCE_ID: test-source\nTEXT: Test evidence."],
    )
    ...
    assert "trust_boundaries" in prompt
    assert "threats" in prompt
    assert "residual_risk_questions" in prompt